...
class Rewe extends Generic {
  ...
  getPageType() {
    if ($(".style_BreadcrumbLinkWrapper__2kavF").length > 0) {
      console.log("ProductOverviewPaige");
  ...
  getBadgeParent() {
    console.log("Parent badge working-Anshaj")
    $('.pdpr-BadgesBar--productInformation').append('<div class = "newWrapper">' + "</div>");
  ...
  async getProductData(customBody = false) {
    ...
    const sizeMatch =
      ...
      regex.exec($body.find(".pdpr-Title").text());
    return {
      category,
      name: $body
        .find(".pdpr-Title")
      ...
      size: sizeMatch
        ...
        : false,
      img: $body.find(".pdsr-ResponsiveImage img").attr("src"),
    };
  }
  ...
  getProductDataFromOverview(element) {
    ...
    var price = element.find(".search-service-productOfferOriginalPrice")
      .text().split('€')[0]
      .replace("€", "")
      .replace(".-", "")
      .replace("-", "")
  ...
  getListItemsNumber() {
    console.log("tiles working fine")
  ...
  getBadgeParentinOverview(element) {
    // let newelement = document.getElementsByClassName("search-service-productDetailsFill style_productDetailsFill__1DoRB");

    // $(newelement).append('<div class = "newWrapper1">' + "</div>");
    // $(".style_productDetailsFill__1DoRB").css({ height: 50 })
    // return $(".newWrapper1");
  ...
  getFilterButton(element){
    let filterButton = element.find(".style_rsFacetGroupTitleContainer__Cp54L")
  }

  getAddToCartButtonInOverview(element) {
    return element.find(".bs_amount-handler");
