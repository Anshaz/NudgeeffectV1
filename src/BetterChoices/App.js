  ...
  async init(group = "C") {
    ...
    switch (pageType) {
      case this.store.pageTypes.SINGLEPRODUCTPAGE: // single product
        ...
        addToCartButton.addClass("bfcAltered").css({ height: 40, margin: "-85px -85px" });
        ...
        addToCartButton.on("click", async (e) => {
          ...
          console.log("here is single product data", productData);
          // block if no price
      ...
      case this.store.pageTypes.PRODUCTOVERVIEWPAGE: // overview page
        ...
        var currentpage = $(".style_paginationPage__7uaa_.style_paginationPageActive__2G5n3.style_paginationPageLink__q5snp").text();
        ...
        const reload = async (observeItem) => {
          ...
          switch (observeItem.attr("class")) {
            case "style_rsResultsText__1sFpn":
              currentproduct = observeItem.text();
              console.log("Listnderung" + observeItem.text());
              break;
            case "style_paginationPage__7uaa_ style_paginationPageActive__2G5n3 style_paginationPageLink__q5snp":
          ...
          var listelementTest = document.getElementsByClassName("search-service-product style_product__2Pg5x");
        ...
        const observer2 = new MutationObserver(() => {
          if ($(".style_rsResultsText__1sFpn").length && !($(".style_rsResultsText__1sFpn").text() === currentproduct)) {
            // configure observer
            const observer = new MutationObserver(reload);
            observer.observe($(".style_rsResultsText__1sFpn")[0], {
              subtree: false,
              childList: true,
            });

            reload($(".style_rsResultsText__1sFpn"));
          }
        });
        ...
        const observer3 = new MutationObserver(() => {
          if ($(".style_paginationPageActive__2G5n3").length && (!($(".style_paginationPageActive__2G5n3").text() === currentpage) || !(this.store.categoryString() == kat))) {
            console.log("wir sind im observer:" + $(".style_paginationPageActive__2G5n3").text());
            // configure observer
            const observer = new MutationObserver(reload);
            observer.observe($(".style_paginationPageActive__2G5n3")[0], {
              subtree: false,
              childList: true,
            });

            reload($(".style_paginationPageActive__2G5n3"));
          }
        });
